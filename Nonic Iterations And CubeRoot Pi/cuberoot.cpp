...
int eqn_cubic(const double *a, double *x) { // Remark #2
	...
	const double *pa, prec = 1.0e-6;
// termination criterion, Remark #3
	if (a[3] == 0.0) {
		if (a[2] == 0.0) {
			if (a[1] == 0.0)
				return 0;
...
// deflation
		c[0] = c[1] * xh + b[1];
		return 1 + eqn_quadratic(c, x, b, i_loc, i_loc + 1);
	}
	return 1;
}
*/
